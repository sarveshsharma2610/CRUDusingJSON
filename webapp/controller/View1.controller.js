sap.ui.define([
	...
	return Controller.extend("crudusingjson.controller.View1", {
		onInit: function(oEvent) {
			...
			var statusModel1 = new JSONModel({
				"reference": [{"CustomerReference": "A"}, {"CustomerReference": "B"}, {"CustomerReference": "C"}]
			});
		...
		onEdit: function(oEvent){
			...
			if(selectedRowData.length === 0)
			{
				//this condiction check wheather the records got selected or not
				MessageToast.show("please select atleast one row");
				return;
			}
			else
		...
		onSave: function(oEvent){
			...
			if (this.mode === "Edit")
			{
				//if user click on save in edit functionality
				var oTable = this.byId("idSalesTable");
				var selectedRowData = oTable.getSelectedContexts();
				selectedRowData.forEach(function(item) {
					var sContext = item;
			...
			if(this.mode === "Add")
			{
				...
				else
				{
					...
					{
						if(sData[i].editable === true)
						{
							//check feilds which are Appended by click on Add
							if (sData[i].SalesOrder === "")
							{
								//check if the entered data is black then it throws a error message
